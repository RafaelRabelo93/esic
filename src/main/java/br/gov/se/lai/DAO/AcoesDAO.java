...
public class AcoesDAO {
    ...
    public static void delete(Acoes acoes) {        
        try {            
            ...
            if (acao == null) {
            	FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error", "Acoes  n„o cadastrado."));
			}else {
				em.getTransaction().begin();
				em.remove(acao);
	            em.getTransaction().commit();
	            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Sucesso", "Acoes  exclu√≠da com sucesso!"));
			}            
        } catch (Exception e) {
            em.getTransaction().rollback();
