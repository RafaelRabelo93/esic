...
public class FiltrarDadosRelatorioEstatico implements Serializable {
	...
	public static Map<String, ArrayList<Integer>> gerarAcompanhamentoDinamico(String query, ArrayList<String> baseRecebida, String tempo, Map<String, ArrayList<String>> dados) {
			...
			if (!dados.get(tempo).get(0).equals("Todos")) {
				...
				if (tempo.equals("mes")) {
					for (String t : keysValues) {
						...
						aux.add(SolicitacaoDAO.queryDinamica(queryTempo).size());
						dadosRelacionadorBase.add(aux);
					}
				} else {
					for (String t : keysValues) {
						base.add(t);
						...
						ArrayList<Integer> aux = new ArrayList<>();
						List<String> auxQuery = SolicitacaoDAO.queryDinamica(queryTempo);
		...
		if(!dados.containsKey(tempo) || dados.get(tempo).get(0).equals("Todos") ) {
			base = baseRecebida;
			String queryTempo = query;
			//verficia se for ano o parametro escolhido, então se tem meses
			for(String t : base) {
				if(tempo.equals("ano")) {
					if(dados.containsKey("mes") && dados.get("mes") != null) {
						keysValues = dados.get("mes");
